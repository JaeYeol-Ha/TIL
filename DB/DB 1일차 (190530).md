...
RDBMS의 주요 목적 : Transaction 처리 

트랜잭션 : 분리되어 수행될 수 없는 작업 단위 (= Unit of Work , 원자성)  
...
SQL처리에 사용되는 메모리가 있다. library cache
...
2. library cache
3. 테이블 정보, 컬럼, 권한 등 checking
   - user table 컬럼 제약조건 = meta 정보 , 시스템 카탈로그, data dictionary
   - semantic checking
4. optimizer (비용 선택(무료인 곳, 가장 빠른 곳 등))
...
select sal, comm, (sal+nvl(comm, 0))*2 as "Salary" from emp; -- " "로 컬럼 대소문자 구분가능하다.

select sal, comm, (sal+nvl(comm, 0))*2 "Total Salary" from emp;  --as 생략 가능, " "로 공백 포함 가능 
...
data 타입 컬럼 : date ± n , date - date, date ± /n

select sal+100, sal-100, sal*2, sal/100 from emp; -- 가공된 결과 리턴

select sal, comm, (sal + comm)*2 from emp;
...
select ename || job from emp; --문자열 결합

select ename || ' is work as' || job from emp; --문자열 결합
...
-> https://docs.oracle.com/cd/E11882_01/server.112/e41084/functions002.htm#SQLRF20032>
...
hiredate 는 date 타입인데 오라클 서버에서 문자열로 자동 형변환 , date타입으로 비교하는 것이 아닌 문자열로 비교한다.
...
select distinct deptno,  job from emp; --distinct는 컬럼 맨 앞에만
