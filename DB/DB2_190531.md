...
number - 산술연산
char/varchar2 - || 결합연산자
date - ±n, ±1/24, date-date(빼기)



**-- 그룹함수를 적용한 컬럼과 그룹함수를 적용하지 않은 컬럼이 select절에 함께 선언될 경우 반드시 그룹함수를 적용하지 않은 컬럼은 group by 절에 선언해야 한다.**



--group by 절에는 column 명만 선언할 수 있다.
...
from emp ----1
where ~ ---2 (filter 조건) xxxxxxxxxxxxxxxx
group by deptno; ----3
```



※ 그룹함수의 조건은 having 절에 선언합니다. having 절은 group by 절과 함께 사용해야 한다.
...
self-join (자기 참조가 가능한 테이블에서만)

※ 조인 조건을 잘못 정의하거나, 조인 조건을 누락하면 cartesian product()
